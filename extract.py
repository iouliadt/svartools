...
def extract_GBCoords():
    ...
    coordinates = {}
    # * Parse genbank file
    gbFile = SeqIO.parse(svt.args.genbank, "genbank")
    ...
    if gbRef not in fasta_dict.keys():
        print("{} was not found in {}".format(gbRef, svt.args.alignment))
        exit()
        ...
        if svt.args.gene in key:
            ...
            if type(coordinates[key]) == CompoundLocation:
                for i, f in enumerate(coordinates[key].parts):
                    loc.append(FeatureLocation(positions[coordinates[key].parts[i].start], positions[coordinates[key].parts[i].end]))
                new_coords[key] = CompoundLocation(loc)
            else:
                new_coords[key] = FeatureLocation(positions[coordinates[key].parts[0].start], positions[coordinates[key].parts[0].end])
    print(new_coords)
    ...
    for key in new_coords:
        ...
        if svt.args.outfile:
            fileName = "{}.fasta".format(svt.args.outfile)
        # Default file name
        else:
...
def extract_customCoords():
    ...
    # Split gene coords
    coords = svt.args.coordinates.split(':')
    start = int(coords[0])
    end = int(coords[1])
